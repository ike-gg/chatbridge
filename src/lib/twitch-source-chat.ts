...
export class TwitchSourceChat {
	...
	constructor({
		displayChannelName,
		sourceChannelName,
	}: { sourceChannelName: string; displayChannelName: string }) {
	...
	private setupMessageListener() {
		this.ws.onmessage = (event) => {
			...
			const messageBody =
				properties
					.find((property: string) => property.startsWith("user-type="))
					?.split("=")[1] ?? "";

			const messageContent =
				messageBody.split("PRIVMSG")[1].trim().split(":")[1].trim() ?? "";
	...
	private async subscribeChannels(channelName: string) {
		const annonymousUsername = `justinfan${Math.floor(Math.random() * 1000000)}`;
